...
class DmsModel {
  constructor(config) {
    this.config = config
    this.rawApi = config.get('RAW_API_URL')
  }
  ...
  async getBlobURL(owner, name, ref, path){
    const rawURL = this.rawApi+owner+'/'+name+'/'+ref+'/'+path
  ...
  async getPackage(owner, name, ref) {
    let path = this.rawApi+owner+'/'+name+'/'+ref+'/datapackage.json'

    let datapackage = await fetch(path, {
        method: 'GET'
    })
    ...
    for(let i=0; i < datapackage.resources.length; i++){
        const filePath = datapackage.resources[i].path
        datapackage.resources[i].download_url = await this.getBlobURL(owner, name, ref, filePath)
    }
