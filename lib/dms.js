...
class DmsModel {
  ...
  async getBlobLFSDownloadURL(lfsServer, oid, size){
    ...
    const lfsDownloadURL = responseJSON.objects[0].actions.download.href
  ...
  async getBlobURL(owner, name, ref, path){
    ...
    if (response.substring(0, 30).includes('git-lfs')){
        ...
        const url = await this.getBlobLFSDownloadURL(lfsServer, oid, size)
        return url
    }
    return url
  }
